...
% TODO: Clean up maybe move second half to elementarty functions
...
This allows us to define $\sin$ and $\cos$ in terms of the complex exponentional as
...
\begin{proof}[Proof of Euler's formula]
  Let $f(\theta) = \cos\theta + i\sin\theta$. Since both $\sin$ and $\cos$ satisfy the second order equation,
  \begin{equation}
...
\subsection{Complex Exponenital}%
\label{sub:complex_exponenital}
...
Can the familiar logarithm function in the real numbers also be generalized easily in the complex numbers. Define a function $w=f(z)$ such that $z = e^w$ with $e$ defined with infinite series.

With $z = re^{i\theta}$ and $w = u + iv$. We see that 
